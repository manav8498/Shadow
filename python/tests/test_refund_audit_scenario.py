...
class TestFixtureIntegrity:
    ...
    def test_candidate_turn1_has_simultaneous_tool_calls(self) -> None:
        ...
        tool_names = [
            c["name"] for c in turn1_content if c.get("type") == "tool_use"
        ]
    ...
    def test_candidate_turn2_contains_processed_successfully(self) -> None:
        ...
        turn2_text = " ".join(
            c.get("text", "")
            for c in responses[1]["payload"]["content"]
            if c.get("type") == "text"
        )
...
class TestSafetyVerdict:
    ...
    def test_fingerprint_drift_detection_result_is_bool(
    ...
    def test_sprt_detects_latency_drift(self, result: AuditResult) -> None:
        assert result.sprt_decision == "h1", (
            f"SPRT should reject H0 (latency drift detected). "
...
class TestPolicyViolations:
    ...
    def test_simultaneous_lookup_refund_violation_caught(
        self, result: AuditResult
    ) -> None:
    ...
    def test_unsolicited_refund_announcement_violation_caught(
        self, result: AuditResult
    ) -> None:
    ...
    def test_simultaneous_violation_is_critical_severity(
        self, result: AuditResult
    ) -> None:
        v = next(
            (v for v in result.policy_violations
             if v.rule_id == "no-simultaneous-lookup-refund"),
            None,
        )
        assert v is not None
        assert v.severity == "critical"

    def test_announcement_violation_is_error_severity(
        self, result: AuditResult
    ) -> None:
        v = next(
            (v for v in result.policy_violations
    ...
    def test_simultaneous_violation_n_violations_positive(
        self, result: AuditResult
    ) -> None:
        v = next(
            (v for v in result.policy_violations
    ...
    def test_announcement_violation_first_turn_is_set(
        self, result: AuditResult
    ) -> None:
        v = next(
            (v for v in result.policy_violations
    ...
    def test_latency_delta_is_positive(self, result: AuditResult) -> None:
        ...
        if latency_delta is not None:
            assert latency_delta > 0, (
                f"Latency delta should be positive (candidate is slower). "
                f"Got: {latency_delta}"
            )
...
class TestConformalCoverage:
    ...
    def test_conformal_q_hat_values_are_non_negative(
        self, result: AuditResult
    ) -> None:
        assert result.conformal is not None
...
class TestBaselineVsBaseline:
    ...
    def test_identical_traces_are_safe(self, result: AuditResult) -> None:
        assert result.is_safe(), (
            "Comparing a trace against itself should always be safe."
        )

    def test_identical_traces_no_policy_violations(
        self, result: AuditResult
    ) -> None:
        critical = [
            v for v in result.policy_violations if v.severity == "critical"
        ]
        assert not critical, (
    ...
    def test_identical_traces_no_fingerprint_drift(
        self, result: AuditResult
    ) -> None:
...
class TestReportRendering:
    ...
    def test_render_report_contains_fail_verdict(
        self, result: AuditResult
    ) -> None:
        report = render_report(result)
        assert "FAIL" in report

    def test_render_report_contains_simultaneous_violation(
        self, result: AuditResult
    ) -> None:
        report = render_report(result)
        assert "no-simultaneous-lookup-refund" in report

    def test_render_report_contains_announcement_violation(
        self, result: AuditResult
    ) -> None:
        report = render_report(result)
        assert "no-unsolicited-refund-announcement" in report

    def test_render_report_contains_fingerprint_section(
        self, result: AuditResult
    ) -> None:
        report = render_report(result)
        assert "BEHAVIORAL FINGERPRINT" in report or "Hotelling" in report

    def test_render_report_contains_sprt_section(
        self, result: AuditResult
    ) -> None:
        report = render_report(result)
        assert "SPRT" in report or "SEQUENTIAL" in report

    def test_render_report_contains_conformal_section(
        self, result: AuditResult
    ) -> None:
        report = render_report(result)
    ...
    def test_render_report_has_dimension_deltas(
        self, result: AuditResult
    ) -> None:
        report = render_report(result)
        ...
        assert any(
            dim in report
            for dim in ["latency", "safety", "tool_call_rate", "verbosity"]
        )
...
class TestJsonSerialization:
    ...
    def test_to_dict_policy_violations_are_dicts(
        self, result: AuditResult
    ) -> None:
        d = result.to_dict()
    ...
    def test_to_dict_conformal_is_dict_or_none(
        self, result: AuditResult
    ) -> None:
        d = result.to_dict()
        assert d["conformal"] is None or isinstance(d["conformal"], dict)

    def test_roundtrip_json_preserves_is_safe(
        self, result: AuditResult
    ) -> None:
        d = result.to_dict()
        serialized = json.dumps(d)
...
class TestCustomPolicies:
    ...
    def test_run_with_subset_policies(self) -> None:
        ...
        subset = [p for p in REFUND_POLICIES
                  if p.id == "no-unsolicited-refund-announcement"]
...
class TestStatisticalInvariants:
    ...
    def test_policy_violations_are_summary_instances(
        self, result: AuditResult
    ) -> None:
        for v in result.policy_violations:
            assert isinstance(v, PolicyViolationSummary)

    def test_dimension_deltas_keys_are_strings(
        self, result: AuditResult
    ) -> None:
        for k in result.dimension_deltas:
            assert isinstance(k, str)

    def test_dimension_deltas_values_are_floats(
        self, result: AuditResult
    ) -> None:
